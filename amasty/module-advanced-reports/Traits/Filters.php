        ...
        switch ($interval) {
            case 'year':
                $collection->getSelect()
                    ->columns([
                        'period' => sprintf('YEAR(CONVERT_TZ('.$tablePrefix. '.' .$dateFiled.', \'+00:00\', \'%s\'))', $timezoneOffsetInUTC),
                    ])
                ->group([sprintf('YEAR(CONVERT_TZ('.$tablePrefix. '.' .$dateFiled.', \'+00:00\', \'%s\'))', $timezoneOffsetInUTC)]);
                break;
            case 'month':
                $collection->getSelect()
                    ->columns([
                        'period' => "DATE_FORMAT(".sprintf('CONVERT_TZ('.$tablePrefix. '.' .$dateFiled.', \'+00:00\', \'%s\')', $timezoneOffsetInUTC).",'%Y-%m')",
                    ])
                    ->group([sprintf('MONTH(CONVERT_TZ('.$tablePrefix. '.' .$dateFiled.', \'+00:00\', \'%s\'))', $timezoneOffsetInUTC)]);
                break;
            case 'week':
                $collection->getSelect()
                    ->columns([
                        'period' => "CONCAT(ADDDATE(".sprintf('DATE(CONVERT_TZ('.$tablePrefix. '.' .$dateFiled.', \'+00:00\', \'%s\'))', $timezoneOffsetInUTC).", "
                            . "INTERVAL 1-".sprintf('DAYOFWEEK(CONVERT_TZ('.$tablePrefix. '.' .$dateFiled.', \'+00:00\', \'%s\'))', $timezoneOffsetInUTC)." DAY), "
                            . "' - ', ADDDATE(".sprintf('DATE(CONVERT_TZ('.$tablePrefix. '.' .$dateFiled.', \'+00:00\', \'%s\'))', $timezoneOffsetInUTC).", "
                            . "INTERVAL 7-".sprintf('DAYOFWEEK(CONVERT_TZ('.$tablePrefix. '.' .$dateFiled.', \'+00:00\', \'%s\'))', $timezoneOffsetInUTC)." DAY))",
                    ])
                    ->group([sprintf('WEEK(CONVERT_TZ('.$tablePrefix. '.' .$dateFiled.', \'+00:00\', \'%s\'))', $timezoneOffsetInUTC)]);
                break;
            case 'day':
            default:
                $collection->getSelect()
                    ->columns([
                        'period' => sprintf('DATE(CONVERT_TZ('.$tablePrefix. '.' .$dateFiled.', \'+00:00\', \'%s\'))', $timezoneOffsetInUTC),
                    ])
                    ->group([sprintf('DATE(CONVERT_TZ('.$tablePrefix. '.' .$dateFiled.', \'+00:00\', \'%s\'))', $timezoneOffsetInUTC)]);
        }
    }
