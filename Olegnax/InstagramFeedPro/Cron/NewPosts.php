...
class NewPosts
{
    ...
    public function execute()
    {
        ...
        if (empty($b)
            ...
            || $b->data->the_key !== $this->helper->getSystemDefaultValue(Helper::OPT_PREFIX . 'code')
        ...
        if ($collectionUsers && $collectionUsers->getSize()) {
            ...
            foreach ($users as $user) {
                ...
                $cursor = $this->getBeforeAfter($user);
                $result = [
                    'added' => 0,
                    'update' => 0,
                    'paging' => $cursor,
                ];
                foreach (['before', 'after'] as $cursorItem) {
                    try {
                        $data = $result;
                        while (isset($data['paging'][$cursorItem])) {
                            $data = 'before' == $cursorItem
                                ? $this->request($user, '', $data['paging'][$cursorItem])
                                : $this->request($user, $data['paging'][$cursorItem]);
     ...
     * @return array
     */
    private function getBeforeAfter($user)
    {
        $before = null;
        $after = null;
        $itemCollection = $user->getAllPosts()->addFieldToFilter(IntsPostData::CODE, ['neq' => 'NULL']);
