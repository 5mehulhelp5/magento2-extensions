    ...
    private function getOptionSwatchHtml($option, $optionValue, $loadLinkedProductLinks, $selectedSameSkuValues)
    {
        ...
        switch ($type) {
            ...
            case 'color':
                ...
                if ($type == 'color') {
                    $swatchImgWidth  = $swatchWidth != 0 ? $swatchWidth : 64;
                    $swatchImgHeight = $swatchHeight != 0 ? $swatchHeight : 64;
                } else {
                    try {
                        $valueSwatchImageSize = getimagesize($value);
                    } catch (\Exception $e) {
                        $valueSwatchImageSize = [$swatchWidth, $swatchHeight];
                    }
                    $swatchImgWidth  = $swatchWidth != 0 ? $swatchWidth : $valueSwatchImageSize[0];
                    $swatchImgHeight = $swatchHeight != 0 ? $swatchHeight : $valueSwatchImageSize[1];
                }
