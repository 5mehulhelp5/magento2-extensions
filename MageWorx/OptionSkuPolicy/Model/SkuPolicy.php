    ...
    protected function processBuyRequestOptions($options)
    {
        ...
        foreach ($options as $optionId => $values) {
            ...
            if (in_array($option->getType(), $this->baseHelper->getSelectableOptionTypes())) {
                ...
                if (is_array($values)) {
                    /**
                     * Multiselect in Hyva added in to BuyRequest like [optionId=>[0=>'val_id1,val_id2,val_id3,...']]
                     */
                    foreach ($values as $value) {
                        if(is_string($value) && str_contains($value, ',')) {
                            $arValues = array_merge($arValues, explode(',', $value));
                            continue;
                        }
                        $arValues[] = $value;
                    }
                ...
                $this->processSelectableOption($option, $optionId, $arValues);
            } else {
